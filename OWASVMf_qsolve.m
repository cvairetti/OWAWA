%% Solving Quadratic Problem with quadsolve

function [Ytest,solvector,bias,nSV] = OWASVMf_qsolve(Y,X,Xt,C,sigma,OWAtype,alphaOWA,quantif,IOWAslack,OWAWAbeta,alphaOWAWA,quantifOWAWA)
...
%     Ytest=(Xt*solvector+bias);   
    Yn=Y.*B/mean(B);
    K=K.*(Yn*Yn');
    [alpha,bias]= quadsolve(K,-(B/mean(B)).*ones(size(K,1),1),Yn',0,C); 
    clear K
    alpha=alpha.*Yn;
    solvector=X'*alpha;
    Ytest=(Xt*solvector-bias);
